                ...
                <tr>
                  <td colSpan={5} className="p-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="text-slate-400 text-xs font-bold uppercase tracking-widest animate-pulse">Syncing courses...</p>
                    </div>
                  </td>
                </tr>
              ) : paginatedData?.length > 0 ? paginatedData.map((course, idx) => (
                <tr key={course.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="p-3 md:p-4 text-center">
                    <div className="flex items-center justify-center gap-3 group/toggle">
                       <span className={`text-[10px] font-bold tracking-wide transition-colors ${course.isActive ? 'text-green-500' : 'text-slate-300'}`}>
                       ...
                       <button 
                          ...
                          className={`w-8 h-4.5 rounded-full relative transition-all duration-300 shadow-inner ${
                             course.isActive ? 'bg-green-500' : 'bg-slate-200'
                          }`}
                          title={course.isActive ? "Hide from portals" : "Activate for portals"}
                       >
                          <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow-md transition-all duration-300 ${
                             course.isActive ? 'right-0.5' : 'left-0.5'
                          }`} />
                       </button>
                    </div>
                  </td>
                ...
                <tr>
                  <td colSpan={5} className="p-16 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="text-sm font-semibold text-slate-600 italic">No courses found matching criteria</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {totalItemsCount > 0 && (
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50 p-4 rounded-xl border border-slate-100 mt-2 text-left">
            <div className="text-[12px] text-slate-500 font-medium order-2 sm:order-1">
                Showing <span className="text-slate-900 font-bold">{startIndex + 1}</span> to <span className="text-slate-900 font-bold">{Math.min(startIndex + pageSize, totalItemsCount)}</span> of <span className="text-slate-900 font-bold">{totalItemsCount}</span> courses
            </div>
